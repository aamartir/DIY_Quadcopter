...
{
  I2C::write8(_dev_address, WHO_AM_I, _addr); 
  _dev_address = _addr;
}

uint8_t ITG3200::getSampleRateDiv() 
{
  return I2C::read8( _dev_address, SMPLRT_DIV );
}

void ITG3200::setSampleRateDiv(uint8_t _SampleRate) 
{
  I2C::write8(_dev_address, SMPLRT_DIV, _SampleRate);
}

uint8_t ITG3200::getFSRange() 
{
  return ((I2C::read8(_dev_address, DLPF_FS ) & DLPFFS_FS_SEL) >> 3);
  //return ((_buff[0] & DLPFFS_FS_SEL) >> 3);
}

void ITG3200::setFSRange(uint8_t _Range) 
{
  I2C::readBytes(_dev_address, DLPF_FS, 1, &_buff[0]);   
  I2C::write8(_dev_address, DLPF_FS, ((_buff[0] & ~DLPFFS_FS_SEL) | (_Range << 3)) ); 
}

uint8_t ITG3200::getFilterBW() 
{  
  return ((I2C::read8(_dev_address, DLPF_FS) & DLPFFS_DLPF_CFG));
...
{   
  I2C::readBytes(_dev_address, DLPF_FS, 1, &_buff[0]);
  I2C::write8(_dev_address, DLPF_FS, ((_buff[0] & ~DLPFFS_DLPF_CFG) | _BW));
}

bool ITG3200::isINTActiveOnLow() 
{  
  return ((I2C::read8(_dev_address, INT_CFG) & INTCFG_ACTL) >> 7);
  //return ((_buff[0] & INTCFG_ACTL) >> 7);
}

void ITG3200::setINTLogiclvl(bool _State) 
{
  I2C::readBytes(_dev_address, INT_CFG, 1, &_buff[0]);
  I2C::write8(_dev_address, INT_CFG, ((_buff[0] & ~INTCFG_ACTL) | (_State << 7))); 
}

bool ITG3200::isINTOpenDrain() 
{  
  return ((I2C::read8(_dev_address, INT_CFG) & INTCFG_OPEN) >> 6);
...
{
  I2C::readBytes(_dev_address, INT_CFG, 1, &_buff[0]);
  I2C::write8(_dev_address, INT_CFG, ((_buff[0] & ~INTCFG_OPEN) | _State << 6)); 
}

bool ITG3200::isLatchUntilCleared() 
{    
  return ((I2C::read8(_dev_address, INT_CFG) & INTCFG_LATCH_INT_EN) >> 5);
...
{
  I2C::readBytes(_dev_address, INT_CFG, 1, &_buff[0]);
  I2C::write8(_dev_address, INT_CFG, ((_buff[0] & ~INTCFG_LATCH_INT_EN) | _State << 5)); 
}

bool ITG3200::isAnyRegClrMode() 
{    
  return ((I2C::read8(_dev_address, INT_CFG) & INTCFG_INT_ANYRD_2CLEAR) >> 4);
...
{
  I2C::readBytes(_dev_address, INT_CFG, 1, &_buff[0]);
  I2C::write8(_dev_address, INT_CFG, ((_buff[0] & ~INTCFG_INT_ANYRD_2CLEAR) | _State << 4)); 
}

bool ITG3200::isITGReadyOn() 
{   
  return ((I2C::read8(_dev_address, INT_CFG ) & INTCFG_ITG_RDY_EN) >> 2);
...
{
  I2C::readBytes(_dev_address, INT_CFG, 1, &_buff[0]);
  I2C::write8(_dev_address, INT_CFG, ((_buff[0] & ~INTCFG_ITG_RDY_EN) | _State << 2)); 
}

bool ITG3200::isRawDataReadyOn() 
{
  return (I2C::read8(_dev_address, INT_CFG) & INTCFG_RAW_RDY_EN);
...
{
  I2C::readBytes(_dev_address, INT_CFG, 1, &_buff[0]);
  I2C::write8(_dev_address, INT_CFG, ((_buff[0] & ~INTCFG_RAW_RDY_EN) | _State)); 
}

bool ITG3200::isITGReady() {
  return ((I2C::read8(_dev_address, INT_STATUS) & INTSTATUS_ITG_RDY) >> 2);
  //return ((_buff[0] & INTSTATUS_ITG_RDY) >> 2);
}

bool ITG3200::isRawDataReady() 
{
  return (I2C::read8(_dev_address, INT_STATUS) & INTSTATUS_RAW_DATA_RDY);
  //return (_buff[0] & INTSTATUS_RAW_DATA_RDY);
}

void ITG3200::readTemp(double *_Temp) {
  I2C::readBytes(_dev_address, TEMP_OUT,2,_buff);
...
void ITG3200::readGyro()
{
  ...
  I2C::readBytes(_dev_address, GYRO_XOUT, 6, _buff);
...
void ITG3200::reset() 
{     
  I2C::write8(_dev_address, PWR_MGM, PWRMGM_HRESET); 
  delay(GYROSTART_UP_DELAY); //gyro startup 
}

bool ITG3200::isLowPower()
{   
  return ((I2C::read8(_dev_address, PWR_MGM) & PWRMGM_SLEEP) >> 6);
  //return (_buff[0] & PWRMGM_SLEEP) >> 6;
}
  
void ITG3200::setPowerMode(bool _State) 
{
  I2C::readBytes(_dev_address, PWR_MGM, 1, &_buff[0]);
  I2C::write8(_dev_address, PWR_MGM, ((_buff[0] & ~PWRMGM_SLEEP) | _State << 6));  
}

bool ITG3200::isXgyroStandby() 
{
  return ((I2C::read8(_dev_address, PWR_MGM) & PWRMGM_STBY_XG) >> 5);
  //return (_buff[0] & PWRMGM_STBY_XG) >> 5;
}

bool ITG3200::isYgyroStandby() 
{
  return ((I2C::read8(_dev_address, PWR_MGM) & PWRMGM_STBY_YG) >> 4);
  //return (_buff[0] & PWRMGM_STBY_YG) >> 4;
}

bool ITG3200::isZgyroStandby() {
  return ((I2C::read8(_dev_address, PWR_MGM) & PWRMGM_STBY_ZG) >> 3);
...
{
  I2C::readBytes(_dev_address, PWR_MGM, 1, &_buff[0]);
  I2C::write8(_dev_address, PWR_MGM, ((_buff[0] & PWRMGM_STBY_XG) | _Status << 5));
}

void ITG3200::setYgyroStandby(bool _Status) 
{
  I2C::readBytes(_dev_address, PWR_MGM, 1, &_buff[0]);
  I2C::write8(_dev_address, PWR_MGM, ((_buff[0] & PWRMGM_STBY_YG) | _Status << 4));
}

void ITG3200::setZgyroStandby(bool _Status) 
{
  I2C::readBytes(_dev_address, PWR_MGM, 1, &_buff[0]);
  I2C::write8(_dev_address, PWR_MGM, ((_buff[0] & PWRMGM_STBY_ZG) | _Status << 3));
}

uint8_t ITG3200::getClockSource() 
{  
  return (I2C::read8(_dev_address, PWR_MGM) & PWRMGM_CLK_SEL);
...
{   
  I2C::readBytes(_dev_address, PWR_MGM, 1, &_buff[0]);
  I2C::write8(_dev_address, PWR_MGM, ((_buff[0] & ~PWRMGM_CLK_SEL) | _CLKsource)); 
}
