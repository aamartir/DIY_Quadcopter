...
void Radio_PCINT::readReceiverData()
{
  ...
  if(data_ready)
  {
    //if( radio.radio_data[RADIO_THROTTLE_CH] >= THTLE_MIDDLE && throttle <= MOTOR_MIN )
    //  return; 
    ...
    target[ROLL]  = map(data_rdy_buffer[RADIO_ROLL_CH],     ROLL_MIN,  ROLL_MAX,  -40, 	       40);     // ROLL
    target[PITCH] = map(data_rdy_buffer[RADIO_PITCH_CH],    PITCH_MIN, PITCH_MAX, -40,         40);     // PITCH
...
ISR( PCINT2_vect )
{
  ...
  //if(((PIND >> RADIO_INT_PIN) & 0x1) == 0) /* Next one will be the beginning of the frame */
  if(!radio.data_ready && ((PIND & RADIO_PIN_MASK) != last_pin_state) ) /* Only fill buffer if data NOT used yet */
  {
    if( (last_pin_state = (PIND & RADIO_PIN_MASK)) == 0 ) /* If signal is LOW */
    {
      ...
      radio.setState( RADIO_SYNCH_START_FRAME );
      frameStarted = 0;
            ...
            diff >= MIN_PW_US)
    {
      if(frameStarted)
      {
        ...
        {
          radio.setState( RADIO_SYNCH_ERROR );
          //SET_BIT(radio.state, RADIO_SYNCH_ERROR);
        }
        else
        {  
          ...
          if(radio.chCounter == RADIO_LAST_CHAN_INDEX)
          {
            radio.setState( RADIO_SYNCH_END_FRAME );
