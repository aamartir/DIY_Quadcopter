...
#define GYROSTART_UP_DELAY  100    // 50ms from gyro startup + 20ms register r/w startup
...
public:
  bool initialized;
  double scalefactor[3];    // Scale Factor for gain and polarity
  double offsets[3];
  int sign[3];
  ...
  void setScaleFactor(double _Xcoeff, double _Ycoeff, double _Zcoeff, bool _Radians);  // negative ciefficient = Reversed
  ...
  void readGyro(double *_GyroX, double *_GyroY, double *_GyroZ); // deg/sec calibrated & ScaleFactor 
